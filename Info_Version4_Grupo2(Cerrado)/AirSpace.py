...
class AirSpace:
    ...
    def load_navpoints(self, filename): #Cargar nodos.
    ...
    def load_navsegments(self, filename): #Cargar segmentos.
        with open(filename, 'r') as f:
            for line in f:
                parts = line.strip().split()
                if len(parts) != 3:
    ...
    def load_navairports(self, filename): #Cargar aeropuertos.
        with open(filename, 'r') as f:
    ...
    def load_all(self, nav_file, seg_file, aer_file): #Función para cargar los archivos.
        self.load_navpoints(nav_file)
        self.load_navsegments(seg_file)
        self.load_navairports(aer_file)

    def get_navpoint_by_number(self, number): #Funciones para asociar navpoints por nombre o número.
